...
copyoutput_to_snipr_projectlevel(){
projectId=${1} #str(sys.argv[1])
input_dirname=${2} #str(sys.argv[2])
resource_dirname=${3}  #str(sys.argv[3])
file_suffix=${4} #str(sys.argv[4])

echo " I AM IN copyoutput_to_snipr_projectlevel "
python3 -c "
import sys 
sys.path.append('/software');
from download_with_session_ID import *; 
uploadfile_projectlevel()" ${projectId} ${input_dirname} ${resource_dirname} ${file_suffix}  ###${file_suffix}  # ${infarctfile_present}  ##$static_template_image $new_image $backslicenumber #$single_slice_filename
